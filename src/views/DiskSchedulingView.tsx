...
export const DiskSchedulingView: React.FC = () => {
  ...
  return (
    <div className="space-y-8 animate-fade-in font-sans">
      <div className="soft-card p-6 rounded-2xl flex flex-wrap items-center justify-between gap-4 border-l-4 border-l-amber-500">
        <div>
          <div className="flex items-center space-x-2">
            <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight">Disk Scheduling &amp; Head Trajectory</h1>
        ...
        <div className="flex items-center space-x-2 text-xs font-mono text-amber-700 bg-amber-50 border border-amber-200 px-3 py-1.5 rounded-xl font-bold">
      ...
      <div className="soft-card p-6 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs font-mono text-amber-700 font-bold bg-amber-50 border border-amber-200 px-3 py-1 rounded-xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 font-mono text-xs">
          <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 space-y-1">
            <span className="text-slate-400">1. Disk Cylinder Range:</span>
            <div className="text-slate-900 font-extrabold text-sm">0 to 130</div>
          </div>

          <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 space-y-1">
            <span className="text-slate-400">2. Initial Head Position:</span>
            <div className="text-amber-600 font-extrabold text-sm">Cylinder #{initialHead}</div>
          </div>

          <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 space-y-1">
            <span className="text-slate-400">3. Official Request Queue:</span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 font-mono text-xs">
        <div className="soft-card p-4 rounded-xl space-y-1 border-l-4 border-l-amber-500">
          <span className="text-slate-400">Official Algorithm</span>
          <div className="text-xl font-extrabold text-slate-900">FCFS Disk Scheduling</div>
        ...
        <div className="soft-card p-4 rounded-xl space-y-1 border-l-4 border-l-blue-500">
          ...
          <div className="text-xl font-extrabold text-blue-600">Cylinder 65</div>
          <span className="text-[10px] text-slate-500">Starting Point</span>
        </div>

        <div className="soft-card p-4 rounded-xl space-y-2 border-l-4 border-l-amber-500">
          <span className="text-slate-400 block font-bold">5. Total Head Movement</span>
          <div className="text-2xl font-extrabold text-amber-600">{activeResult.totalSeekDistance} <span className="text-xs text-slate-400 font-normal">tracks</span></div>
          <button
        ...
        <div className="soft-card p-4 rounded-xl space-y-2 border-l-4 border-l-emerald-500">
          <span className="text-slate-400 block font-bold">6. Average Seek Distance</span>
          <div className="text-2xl font-extrabold text-emerald-600">{activeResult.avgSeekDistance} <span className="text-xs text-slate-400 font-normal">tracks/req</span></div>
          <button
      ...
      <div className="soft-card p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 font-mono text-xs">
          {activeResult.trajectory.map((step) => (
            <div key={step.stepIndex} className="p-3.5 bg-slate-50 rounded-xl border border-slate-200 flex items-center justify-between">
              <div className="space-y-0.5">
                <div className="font-bold text-slate-900 text-sm flex items-center space-x-2">
                  <span>Step {step.stepIndex}: {step.fromCylinder} &rarr; {step.toCylinder}</span>
                </div>
                <div className="text-[11px] text-slate-500">Seek = |{step.toCylinder} - {step.fromCylinder}| = <strong className="text-amber-600">+{step.seekDistance} tracks</strong></div>
              </div>
              <div className="text-right">
                <span className="text-[10px] text-slate-400 block">Cumulative</span>
                <span className="text-blue-600 font-bold">{step.cumulativeSeek} tracks</span>
              </div>
            </div>
          ))}
        </div>

        <div className="p-3.5 bg-amber-50 border border-amber-200 rounded-xl text-xs font-mono text-amber-800 text-center font-bold">
      ...
      <div className="soft-card p-6 space-y-4">
        <h3 className="text-sm font-extrabold text-slate-900 flex items-center justify-between">
          ...
          <span className="text-xs font-mono text-amber-600 font-bold">
            Total Movement: <strong className="text-slate-900 text-sm">{activeResult.totalSeekDistance}</strong> tracks
          </span>
        </h3>
          ...
          <table className="w-full text-left text-xs font-mono">
            <thead>
              <tr className="border-b border-slate-100 text-slate-400 bg-slate-50">
                ...
                <th className="p-3 text-amber-600">Absolute Seek Distance Formula</th>
                <th className="p-3 text-blue-600">Cumulative Seek Tracks</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {activeResult.trajectory.map((step) => (
                <tr key={step.stepIndex} className="hover:bg-slate-50/80">
                  <td className="p-3 font-bold text-slate-700">Step {step.stepIndex}</td>
                  <td className="p-3 text-slate-500">{step.fromCylinder}</td>
                  <td className="p-3 text-amber-700 font-bold">{step.toCylinder}</td>
                  <td className="p-3 text-amber-600 font-bold">
                    |{step.toCylinder} - {step.fromCylinder}| = +{step.seekDistance} tracks
                  </td>
                  <td className="p-3 text-blue-600 font-bold">{step.cumulativeSeek} tracks</td>
                  <td className="p-3 text-slate-500 text-[11px]">{step.lmsOperation}</td>
                </tr>
              ))}
            </tbody>
          </table>
